...
export async function GET(request, { params }) {
    try {
        ...
        const processedFiles = files.map(fileData => {
            const isLocked = fileData.requirements ? checkLockStatus(fileData.requirements, userData) : false;

            return {
                id: fileData.id,
                ...fileData,
                isLocked,
                lockReason: isLocked ? getLockReason(fileData.requirements, userData) : null,
        ...
        const processedVariableFiles = variableFiles ? {
            luckyGenerator: variableFiles.luckyGenerator?.enabled ? {
                ...
                isLocked: !userData.unlockedGenerators?.[params.gameId]?.[variableFiles.luckyGenerator.requiredKey],
                requirements: {
                    type: "purchase",
            ...
            makeYourOwn: variableFiles.makeYourOwn?.enabled ? {
                ...
                isLocked: !userData.unlockedGenerators?.[params.gameId]?.[variableFiles.makeYourOwn.requiredKey],
                requirements: {
                    type: "purchase",
...
function checkLockStatus(requirement, userData) {
    ...
    switch (requirement.type) {
        ...
        case "achievement":
            return !userData.achievements || !userData.achievements.includes(requirement.value);
        case "purchase":
            return !userData.purchasedProducts || !userData.purchasedProducts.includes(requirement.value);
        default:
            return false;
    }
}
...
function getLockReason(requirement, userData) {
    ...
    switch (requirement.type) {
        case "level":
            return `Requires Level ${requirement.value} (Current: ${userData.level || 0})`;
        case "achievement":
