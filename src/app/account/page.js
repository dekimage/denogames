...
import { GamepadIcon, Package, Trophy, Calendar, Star, ShoppingCart, ChevronRight } from "lucide-react";
...
const StatCard = ({ icon: Icon, label, value, total, href, subLabel, showProgress = true }) => (
    <Link href={href}>
...
const AccountOverview = observer(() => {
    ...
    const ownedGames = products.filter(p =>
        p.type === "game" && user.purchasedProducts?.includes(p.id)
    ).length;
    const ownedExpansions = products.filter(p =>
        p.type === "expansion" && user.purchasedProducts?.includes(p.id)
    ).length;
    ...
    const reviewedProductIds = new Set(user.reviews?.map(review => review.productId) || []);
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <MembershipCard
                    title={user.tags?.isActivePatreon ? "Thank you for being a Patron!" : "Patreon Member"}
                    description={
                ...
                <MembershipCard
                    title={user.tags?.isActiveSubstack ? "Thank you for reading Deno Press" : "Subscribe to Deno Press"}
                    description={
                ...
                <MembershipCard
                    title={user.tags?.isActiveDiscord ? "Thank you for being a Discord Member" : "Join our Discord"}
                    description={
